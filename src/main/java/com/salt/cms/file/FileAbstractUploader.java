...
import io.github.talelin.autoconfigure.exception.FileTooLargeException;
 ...
 */
public abstract class FileAbstractUploader implements FileUploader {

    private FileUploadHandler uploadHandler;

    @Override
    public List<File> upload(MultiValueMap<String, MultipartFile> fileMap, String fileType, String id) {
        checkFileMap(fileMap);
        // 得到单个文件的大小限制
        // 本地存储需先初始化存储文件夹
        return handleMultipartFiles(fileMap, fileType,id);
    }

    @Override
    public List<File> upload(MultiValueMap<String, MultipartFile> fileMap,String fileType,String id, FileUploadHandler uploadHandler) {
        this.uploadHandler = uploadHandler;
        return this.upload(fileMap,fileType,id);
    }

    protected List<File> handleMultipartFiles(MultiValueMap<String, MultipartFile> fileMap,String FileType,String id) {
        long singleFileLimit = getSingleFileLimit();
        List<File> res = new ArrayList<>();
        fileMap.keySet().forEach(key -> fileMap.get(key).forEach(file -> {
            if (!file.isEmpty()) {
                handleOneFile0(res, singleFileLimit, file,FileType,id);
            }
        }));
        return res;
    }

    private void handleOneFile0(List<File> res, long singleFileLimit, MultipartFile file,String fileType,String id) {
        ...
        if (ok) {
            ...
            if (uploadHandler != null) {
                uploadHandler.afterHandle(fileData,fileType,id);
            }
        }
    }
